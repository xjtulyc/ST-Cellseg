...
class DensityPeakCluster(object):
    ...
    def build_distance(self):
        ...
        # distance_matrix = fastdist(np.array(self.data, dtype=np.float16), metric=self.distance_metric)
        distance_matrix = fastdist.vector_to_matrix_distance(all_ngc[i, :], all_ngc[j, :], fastdist.euclidean, "euclidean")
    ...
    def fit(self, data):
        ...
        :return: None
        """
        if isinstance(data, np.ndarray): data = np.array(data, dtype=np.float16)
