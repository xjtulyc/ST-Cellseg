...
import stumap.sparse as sparse
...
import stumap.distances as dist

from stumap.utils import (
    submatrix,
    ts,
    csr_unique,
    fast_knn_indices,
)
from stumap.spectral import spectral_layout
from stumap.layouts import (
...
class UMAP(BaseEstimator):
    ...
    def st_fit(self, X, y=None):
        ...
        self._initial_alpha = self.learning_rate  # 1.0

        self.knn_indices = self.precomputed_knn[0]  # None
        self.knn_dists = self.precomputed_knn[1]  # None
        self.knn_search_index = self.precomputed_knn[2]  # None

        self._validate_parameters()  # Verify the validity of the parameter
        ...
        if X[index].shape[0] <= self.n_neighbors:  # 2
        ...
        if self._sparse_data and not X.has_sorted_indices:  # False
            X.sort_indices()
        ...
        if self.metric == "precomputed" and self._sparse_data:  # False
        ...
        elif X[index].shape[0] < 4096 and not self.force_approximation_algorithm:  # False
