...
# from clustermap_distance import get_ClusterMap_distance, spearman_corr
...
def convert(x, y):
    ...
    p_y = y['p']
    msngc_y = y['msngc'].flatten()
    d = np.array([p_y.shape[0]], dtype=np.int8)
    return np.hstack((d, p_x, msngc_x)), np.hstack((d, p_y, msngc_y))
...
def STUMAP_distance(x, y):
    ...
    p_y = y[1:d + 1]
    msngc_y = y[d + 1:]
    return euclidean(p_x, p_y) * (1 - correlation(msngc_x, msngc_y))


@numba.njit(fastmath=True)
def STUMAP_distance_grad(x, y):
    d = x[0]
    p_x = x[1:d + 1]
    msngc_x = x[d + 1:]
    p_y = y[1:d + 1]
    msngc_y = y[d + 1:]
    d = euclidean(p_x, p_y) * (1 - correlation(msngc_x, msngc_y))
    _, m_g = euclidean_grad(p_x, p_y)
    _, c_g = correlation_grad(msngc_x, msngc_y)
    grad = m_g * euclidean(p_x, p_y) \
           - m_g * correlation(msngc_x, msngc_y) \
           - euclidean(p_x, p_y) * c_g
    return d, grad
...
# @numba.njit(nopython=True, parallel=True)
# def clustermap(idx_1, idx_2):
#     r"""
#     clustermap distance
#     :param idx_1: point_idx
#     :param idx_2: point_idx
#     :return: clustermap distance
#     """
#     idx_1, idx_2 = int(idx_1[0]), int(idx_2[0])
#     # print('idx1: ', idx_1)
#     # print('idx2: ', idx_2)
#     points_distance = euclidean(Xdata[idx_1], Xdata[idx_2])
#     ngc_distance = cosine(ngc[idx_1].toarray(), ngc[idx_2].toarray())
#     # ngc_distance = distance2(self.ngc[i], self.ngc[j])
#     cm_distance = None
#     if points_distance == 0 and ngc_distance == 0:
#         cm_distance = 0
#     if points_distance != 0 and ngc_distance == 0:
#         cm_distance = np.inf
#     cm_distance = points_distance / (1e-6 + ngc_distance)
#     return cm_distance
#
#
# # @numba.njit()
# @numba.njit(nopython=True, parallel=True)
# def clustermap_grad(idx_1, idx_2):
#     r"""
#     clustermap distance
#     :param idx_1: point_idx
#     :param idx_2: point_idx
...
#     x = Xdata[idx_1]
#     y = Xdata[idx_2]
#     ngc_x = ngc[idx_1].toarray()
#     ngc_y = ngc[idx_2].toarray()
#     _, points_grad = euclidean_grad(x, y)
#     ngc_grad = euclidean_grad(x, y)
#     corr = cosine(ngc_x, ngc_y)
#     return clustermap(idx_1, idx_2), points_grad / (1e-6 + corr)
...
def euclidean_grad(x, y):
    ...
    for i in range(x.shape[0]):
        result += (x[i] - y[i]) ** 2
    d = np.sqrt(result)  # euclidean distance
    grad = (x - y) / (1e-6 + d)
    return d, grad
...
named_distances = {
    ...
    "STUMAP_distance": STUMAP_distance,
    "euclidean": euclidean,
    "l2": euclidean,
    "manhattan": manhattan,
    "taxicab": manhattan,
...
named_distances_with_gradients = {
    "STUMAP_distance": STUMAP_distance_grad,
    # general minkowski distances
